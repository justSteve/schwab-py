...
def latest_order_main(sys_args):
    parser = argparse.ArgumentParser(
        description='Utilities for generating code from orders via clipboard')
    ...
    client = auth.easy_client(args.api_key, args.app_secret, 'callback_url', args.token_file)
    ...
    #     orders = get_orders(
    #         lambda: client.get_orders_for_account(account_hash))
    #     if orders is None:
    #         return -1
